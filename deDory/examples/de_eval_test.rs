...
use deDory::{
    eval::{compute_evaluation_vector, de_generate_eval_proof, verify_de_eval_proof},
    DeDoryCommitment, PublicParameters, SubProverSetup, VerifierSetup,
};
...
fn test_dedory_helper<E: Pairing>(
    ...
    let b_point = core::iter::repeat_with(|| E::ScalarField::rand(&mut rng))
        ...
        .collect::<Vec<_>>();
    println!(
        "Generating random poly evaluations and points time: {:?}",
        time.elapsed()
    );
    ...
    println!(
        "Generating sub-prover's witness matrix time: {:?}",
        time.elapsed()
    );

    // Commit
    let time = Instant::now();
    let (full_f_comm, sub_T_vec_prime) =
        DeDoryCommitment::deCommit(sub_prover_id, &sub_witness_vec, m, n, &sub_prover_setup);
...
fn new_setup<E: Pairing>(max: usize) -> (SubProverSetup<E>, VerifierSetup<E>) {
    // Setup
    let time = Instant::now();
    let mut rng = ark_std::test_rng();
...
fn new_setup_to_file<E: Pairing>(max: usize) -> (SubProverSetup<E>, VerifierSetup<E>) {
    // Setup
    let time = Instant::now();
    let mut rng = ark_std::test_rng();
    let public_parameters = if Net::am_master() {
    ...
    println!(
        "Sub-Prover setup, writing to and reading from file time: {:?}",
        time.elapsed()
    );
    let time = Instant::now();
    VerifierSetup::new_to_file(&public_parameters, &verifier_setup_filepath);
    println!(
        "Verifier setup, writing to and reading from filetime: {:?}",
        time.elapsed()
    );
...
fn setup_from_file<E: Pairing>(max: usize) -> (SubProverSetup<E>, VerifierSetup<E>) {
    let sub_prover_setup_filepath = format!("../data/SubProver-max{}.paras", max);
    let verifier_setup_filepath = format!("../data/Verifier-max{}.paras", max);
    ...
    let sub_prover_setup = SubProverSetup::read_from_file(&sub_prover_setup_filepath).unwrap();
    println!(
        "Reading sub-prover's paras from file time: {:?}",
        time.elapsed()
    );
    let time = Instant::now();
    let verifier_setup = VerifierSetup::read_from_file(&verifier_setup_filepath).unwrap();
    println!(
        "Reading verifier's paras from file time: {:?}",
        time.elapsed()
    );
